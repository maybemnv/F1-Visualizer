...
Source packages live under `src/`: `src/dashboard/` for UI code (callbacks, components, layout, graphs) and `src/f1_visualization/` for core processing (cache, helpers, plots, schemas, session, ml). `app.py` at root starts the Dash server and `config.py` holds runtime settings. Tests in `tests/` mirror the `src/` hierarchy. Static assets are under `Assets/`, example visuals and notes in `Docs/` and `Comments/`, and CSV/TOML race data in `Data/`.
...
uv run pytest --cov=src/f1_visualization --cov=src/dashboard
...
## Testing Guidelines
Pytest is configured in `pyproject.toml`; place new tests in `tests/` and mirror the `src/` hierarchy, for example `tests/f1_visualization/test_cache.py`. Add focused unit tests for data transforms, schemas, cache behavior, and callback helpers when logic changes. Run coverage when touching shared processing code, but there is no hard minimum currently checked in repo config.
